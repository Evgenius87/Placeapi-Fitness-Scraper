...
async def _search_by_area(city: str):
    ...
    (
      // --- 1. Основні теги ---
      nwr(area.cityArea)["amenity"~"^(gym|dojo|studio|training)$"];
      nwr(area.cityArea)["leisure"~"^(fitness_centre|fitness_station|sports_centre|climbing|swimming_pool)$"];
      nwr(area.cityArea)["sport"~"^(fitness|yoga|pilates|boxing|martial_arts|kickboxing|aikido|karate|judo|taekwondo|kung_fu|crossfit|climbing|bouldering|aerobics|cycling|personal_training|strength_training|stretching|swimming|water_polo|synchronized_swimming|aqua_aerobics)$"];
      nwr(area.cityArea)["club"~"^(sport|fitness|gym|swimming)$"];
      nwr(area.cityArea)["shop"~"^(sports|fitness_equipment)$"];

      // --- 2. Будівлі ---
      nwr(area.cityArea)["building"~"^(fitness_centre|gym|sports_hall|dojo|swimming_pool)$"];

      // --- 3. Назви (fallback) ---
      nwr(area.cityArea)["name"~"(gym|fitness|yoga|pilates|boxing|dojo|pool|swim|aquatic)", i];
    );
    out center;
    """
    return await _query_overpass(query)
...
async def _search_by_coordinates(lat: float, lon: float, radius: int = 50000):
    ...
    (
      // --- 1. Основні теги ---
      nwr(around:{radius},{lat},{lon})["amenity"~"^(gym|dojo|studio|training)$"];
      nwr(around:{radius},{lat},{lon})["leisure"~"^(fitness_centre|fitness_station|sports_centre|climbing|swimming_pool)$"];
      nwr(around:{radius},{lat},{lon})["sport"~"^(fitness|yoga|pilates|boxing|martial_arts|kickboxing|aikido|karate|judo|taekwondo|kung_fu|crossfit|climbing|bouldering|aerobics|cycling|personal_training|strength_training|stretching|swimming|water_polo|synchronized_swimming|aqua_aerobics)$"];
      nwr(around:{radius},{lat},{lon})["club"~"^(sport|fitness|gym|swimming)$"];
      nwr(around:{radius},{lat},{lon})["shop"~"^(sports|fitness_equipment)$"];

      // --- 2. Будівлі ---
      nwr(around:{radius},{lat},{lon})["building"~"^(fitness_centre|gym|sports_hall|dojo|swimming_pool)$"];

      // --- 3. Назви (fallback) ---
      nwr(around:{radius},{lat},{lon})["name"~"(gym|fitness|yoga|pilates|boxing|crossfit|dojo|pool|swim|aquatic)", i];
    );
    out center;
    """
    return await _query_overpass(query)
